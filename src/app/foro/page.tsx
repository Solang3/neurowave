...
export default async function ForoPage({
  ...
  if (activeWave !== 'general') postsQuery = postsQuery.eq('wave', activeWave)
  if (filterUserId) postsQuery = postsQuery.eq('user_id', filterUserId)

  const { data: posts } = await postsQuery

  const { data: filteredUserProfile } = filterUserId ? await supabase
    .from('profiles').select('full_name, username').eq('id', filterUserId).maybeSingle() : { data: null }

  const authorIds = [...new Set(posts?.map((p) => p.user_id) || [])]
  const { data: authorProfiles } = authorIds.length > 0 ? await supabase
    .from('profiles').select('id, full_name, username').in('id', authorIds) : { data: [] }

  const postIds = posts?.map((p) => p.id) || []
  const { data: commentCounts } = postIds.length > 0 ? await supabase
    .from('comments').select('post_id').in('post_id', postIds) : { data: [] }
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-8 pt-24 pb-12">
        ...
        {filterUserId && (
          <div className="bg-surface border border-white/[0.07] rounded-xl px-4 py-3 flex items-center justify-between mb-6">
            <p className="text-sm text-muted">Mostrando posts de <span className="text-white">{filteredUserName}</span></p>
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-muted">{posts?.length || 0} {posts?.length === 1 ? 'hilo' : 'hilos'}</p>
          {user ? (
            <Link href={`/foro/nuevo?wave=${activeWave}`} className="bg-accent text-bg text-sm font-medium px-5 py-2.5 rounded-full hover:opacity-90 transition-opacity">
              + Nuevo hilo
            </Link>
          ) : (
            <Link href="/registro" className="border border-white/10 text-sm px-5 py-2.5 rounded-full hover:border-white/25 transition-colors">
        ...
        <div className="flex flex-col gap-3">
          {!posts?.length && (
            <div className="bg-surface border border-white/[0.07] rounded-2xl p-12 text-center">
              ...
              <p className="text-muted text-sm">{user ? '¡Sé el primero en crear uno!' : 'Registrate para ser el primero en participar.'}</p>
            </div>
          )}
          {posts?.map((post) => {
            const waveData = WAVES.find(w => w.id === post.wave)
            return (
              <Link key={post.id} href={`/foro/${post.id}`} className="bg-surface border border-white/[0.07] hover:border-white/[0.15] rounded-2xl p-6 transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: `${waveData?.color}20`, color: waveData?.color, border: `1px solid ${waveData?.color}30` }}>
